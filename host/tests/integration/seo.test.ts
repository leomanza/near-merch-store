  ...
  getProduct: vi.fn().mockImplementation(({ id }: { id: string }) =>
    Promise.resolve({
      product: {
        ...
        price: 29.99,
        category: "T-Shirts",
        images: [{ url: `https://example.com/images/${id}.png`, type: "preview" }],
        variants: [],
        options: [],
      },
    })
...
describe("SEO Head Extraction", () => {
  ...
  describe("Dynamic Route (/products/:productId)", () => {
    const testProductId = "test-product-123";

    it("includes product title in page title", async () => {
      const head = await routerModule.getRouteHead(`/products/${testProductId}`, {
        assetsUrl: config.ui.url,
        runtimeConfig: {
          env: config.env,
          title: config.title,
      ...
      expect(titleMeta).toBeDefined();
      expect((titleMeta as { title: string }).title).toContain("Near Merch");
    });

    it("includes product title in og:title", async () => {
      const head = await routerModule.getRouteHead(`/products/${testProductId}`, {
        assetsUrl: config.ui.url,
        runtimeConfig: {
          env: config.env,
      ...
      expect(ogTitle).toBeDefined();
      expect((ogTitle as { content: string }).content).toBeTruthy();
    });

    it("has description meta tag", async () => {
      const head = await routerModule.getRouteHead(`/products/${testProductId}`, {
        assetsUrl: config.ui.url,
        runtimeConfig: {
          env: config.env,
    ...
    it("has og:type set to product", async () => {
      const head = await routerModule.getRouteHead(`/products/${testProductId}`, {
        assetsUrl: config.ui.url,
        runtimeConfig: {
          env: config.env,
      ...
      expect(ogType).toBeDefined();
      expect((ogType as { content: string }).content).toBe("product");
    });

    it("has twitter card meta tags", async () => {
      const head = await routerModule.getRouteHead(`/products/${testProductId}`, {
        assetsUrl: config.ui.url,
        runtimeConfig: {
          env: config.env,
